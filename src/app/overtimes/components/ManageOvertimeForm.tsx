      ...
      <Form form={form} onFinish={handleSubmit}>
        ...
        <div className="flex flex-row align-middle" id="form">
          <Form.Item
            name="startedDate"
            colon={false}
            rules={[{ required: true, message: '초과근무 시작일을 입력해주세요' }]}
          >
            <DatePicker
              placeholder="시작일"
              format="YYYY-MM-DD"
          ...
          <Form.Item
            ...
            colon={false}
            rules={[
              { required: true, message: '초과근무 시작 시간을 입력해주세요' },
              { validator: validateEndDate },
              {
                validator: (_, value) => {
                  if (value) {
                    ...
                    if (hours > 23) {
                      return Promise.reject(new Error('시작 시간은 23:59를 초과할 수 없습니다.'));
                    }
                  }
                  return Promise.resolve();
                },
              },
              { pattern: /^[0-2]{1}[0-9]{1}:[0-5]{1}[0-9]{1}$/, message: '잘못된 시간 형식입니다.' },
            ]}
          >
            <Input
              ref={(ref) => withMask('99:99', { placeholder: '' })(ref?.input!)}
              type="text"
              placeholder="시작 시각"
              inputMode="numeric"
        ...
        <div className="flex flex-row align-middle">
          <Form.Item
            ...
            colon={false}
            rules={[
              { required: true, message: '초과근무 종료일을 입력해주세요' },
              { validator: validateEndDate },
            ]}
          >
            <DatePicker
          ...
          <Form.Item
            ...
            colon={false}
            rules={[
              { required: true, message: '초과근무 종료 시간을 입력해주세요' },
              { validator: validateEndDate },
              {
                validator: (_, value) => {
                  if (value) {
                    const [hours, minutes] = value.split(':').map(Number);
                    if (hours > 23) {
                      return Promise.reject(new Error('종료 시간은 23:59를 초과할 수 없습니다.'));
                    }
                  }
                  return Promise.resolve();
                },
        ...
        <Form.Item>
          <Button ghost={false} htmlType="submit" type="primary" loading={loading}>
            요청하기
          </Button>
        </Form.Item>
      </Form>
